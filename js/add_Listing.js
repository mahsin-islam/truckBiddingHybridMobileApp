...
$(document).on("pageshow","#pageAddNewStep2", function()
{
    AxelDiff=1;
});
...
$(document).on("click","#campic", function()
{
    ...
    $(".picup").popup("close");
    navigator.camera.getPicture(cameraSuccess, cameraError, { quality: 30,correctOrientation: true,
...
$(document).on("click","#gallery", function()
{
    ...
    $('#myImage').attr('src','');
    $('#smallImage').attr('src','');
    navigator.camera.getPicture(cameraSuccess, cameraError, { quality: 30,
            destinationType: Camera.DestinationType.FILE_URI,
...
**************************************************************************************************************************************/
 $('#axel_Config').selectmenu();
 $('#diff_Locks').selectmenu();
    ...
    service.ajaxCall(gettrckdetails_Json,function(result_data)
        {
            if(result_data.responseCode==200)
            {
                     ...
                     truckInfo.payment_status = result_data.truckData.paymentStatus
                     ...
                     if(result_data.truckData.furtherDetail.length!=0)
                     {
                        for(i=0;i<result_data.truckData.furtherDetail.length;i++)
                        {
                            ...
                            else if(result_data.truckData.furtherDetail[i].name=="diff_locks")
                            {
...
{
    $("#add_SubCategory").selectmenu();
//    alert('only_lising'+only_lising.truckData.truckDetail.cat_2_name);
//    if(only_lising.truckData.truckDetail.cat_2_name!="")
     if( !((only_lising.truckData.truckDetail.cat_2_name == "") || (only_lising.truckData.truckDetail.cat_2_name=="Sub category")) )
     {
