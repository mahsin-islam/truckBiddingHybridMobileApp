...
/*****************************************************************************************
 ...
 INPUT PARAMETERS   :
 *******************************************************************************************/
function loadCatagories3_AU(textSelected)
{
            ...
            //alert(' textSelected '+textSelected+ '  val  '+valueSelected)
               ...
               else
               {
                 ...
                 $('#catagory3Continer_AU').trigger('create');
                    loadCatagories3_AU(textSelected);
               }
});
...
/*****************************************************************************************
 ...
 INPUT PARAMETERS   :
 ************************************************************************************************************/
function  loadCatagories2_AU(select_string)
{
    var getcatg2_Json={"action":"subCategory",
                       "parent":select_string
                      };
    getcatg2_Json=JSON.stringify(getcatg2_Json);
    ...
    service.ajaxCall(getcatg2_Json,function(result_data){
            try{
                     if(result_data.responseCode == 200 || result_data.responseMessage == 'category Fetched successfully.')
                     {
                        ...
                        for(var i=0;i<result_data.category.length;i++)
                        {
                            //alert(result_data.category[i] + '  '+ i)
                            var tst_var=true;
                            $.each(result_data.category[i],function(idx,val)
                                   {
                                    if(tst_var == true){
                                  ...
                                  //  alert(idx+' --- '+val +'hfjhdsjg '+result_data.category[i].hasSubcategory+' ?? '+ val_Option);
                                    tst_var = false;
...
$(document).on('change','#searchBoxCatagory1_Au',function(){
     ...
     //alert(' textSelected '+textSelected)
       if(textSelected == 'Select a catagory' || textSelected == 'All catagories')
       {
         $('#searchBoxCatagory2_Au').empty();
         $('#searchBoxCatagory2_Au').append('<option disabled selected style="display:none;">Select a sub-category</option>');
       ...
       {
         loadCatagories2_AU(textSelected);
       }
});
...
function loadCatagories1_AU()
{
    ...
    {
         //document.getElementById("searchBox_minprice_AU").value = 00
         //document.getElementById("searchBox_minprice_AU").value = 00
         $("#searchBox_minprice_AU option").each(function () {
    ...
    $("#searchBox_minprice_AU").trigger('create')
...
function loadCatagories1PRivate_AU()
{
   ...
   {
      var text1 = 'Min price';
      $("#searchBox_minprice_AU option[text=" + text1 + "]").attr('selected', 'selected');
       var text2 = 'Max price';
       $("#searchBox_maxprice_AU option[text=" + text2 + "]").attr('selected', 'selected');
                      
     });
    $("#searchBox_minprice_AU").trigger('create');
    $("#searchBox_maxprice_AU").trigger('create');
