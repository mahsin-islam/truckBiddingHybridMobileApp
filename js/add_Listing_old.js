    ...
    service.ajaxCall(getcatg2_Json,function(result_data){
        try{
            if(result_data.responseCode == 200 || result_data.responseMessage == 'category Fetched successfully.')
            {
                ...
                $('#add_ThreeCategory').append('<option disabled selected style="display:none;" value="">Additional Category</option>');
...
function addStep3()
{
    ...
    //alert(truckInfo.addCatagory);
    //alert(truckInfo.addCatagoryId)
...
//                     $('#add_enqPhone').val(result_data.truckData.truckDetail.enquiry_phone);
                    ...
                    else if(result_data.truckData.truckDetail.inc_gst=='0')
                    {
                     ...
                     $('#add_Brakes').val("");
                     $('#add_color').val("");
                     //var stock_Value = Short_StateName(result_data.truckData.truckDetail.state)
                     //alert(stock_Value)
                     //setTimeout(function(){
                     ...
                     else if(result_data.truckData.truckDetail.type=="Dealer Used")
                     {
                        $("#add_Type").val("dealer_used");
                        $("#add_Type").selectmenu('refresh', true);
                     }
                     else if(result_data.truckData.truckDetail.type=="Dealer New")
                     {
                        $("#add_Type").val("dealer_new");
                        $("#add_Type").selectmenu('refresh', true);
                     }
                     else if(result_data.truckData.truckDetail.type=="Dealer Demo")
                     {
                        $("#add_Type").val("dealer_demo");
                        $("#add_Type").selectmenu('refresh', true);
                     }
                     else if(result_data.truckData.truckDetail.type=="Private Used")
                     {
                     ...
                     setTimeout(function(){
                        if(result_data.truckData.truckDetail.model=="")
                        {
                            $("#add_Model").val(result_data.truckData.truckDetail.model);
                            $("#add_Model").selectmenu('refresh', true);
                        }
                        else
                        {
                            ...
                            {
                                if(result_data.truckData.truckDetail.model==idx)
                                {
                                   var modelval = obj;
                                   //alert(modelval);
                     ...
                     setTimeout(function()
                     {
                            ...
                            $("#add_Category").selectmenu('refresh', true);
                            //var CateSelected = $("#add_Category").find("option:selected");
                           // var textCategory   = CateSelected.text();
                            //alert('cate '+textCategory)
                            load_Subcatagory(result_data.truckData.truckDetail.cat_1_id);
                     },300);
                     setTimeout(function()
                     {
                        if(result_data.truckData.truckDetail.cat_3_name=="")
                        {
                            var val_Option = result_data.truckData.truckDetail.cat_2_id+'_0';
                        }
                        else
                        {
                            var val_Option = result_data.truckData.truckDetail.cat_2_id+'_1';
                            $('#add_CategoryContainer').append('<select id="add_ThreeCategory"><option disabled selected style="display:none;">Additional Category</option></select>');
                            $('#add_CategoryContainer').trigger('create');
                        }
                        //alert(val_Option);
                     ...
                     setTimeout(function()
                     {
                        $("#add_ThreeCategory").val(result_data.truckData.truckDetail.cat_3_id);
                        $("#add_ThreeCategory").selectmenu('refresh', true);
