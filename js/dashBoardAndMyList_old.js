...
$(document).on("pageshow", "#pageDashboard", function(){

        //       var urL='https://truckdealersaustralia.com.au/api/search/?type=dealer_new&type=dealer_used&type=dealer_demo';
         //      service.ajaxget(urL, displayDashboard, function(err){
   ...
   // alert(res.hits.hits.length);
    ...
    for(var i=0;i<data.length;i++){
      ...
      //  alert(diffDays);
        ...
        '<a class="trackdetail"><div class="left-sec" onclick="getTruckDetails_AU('+data[i]._id+')"><div class="image"><img src="'+imgurl+'"></div>'+
...
$(document).on("pageshow", "#pageMyListing", function(){
               var jsoN={
                        "action": "myListing",
                        "userID": user.Id,
                        "deviceID": device.Id,
...
function displayMyList(res){
   ...
   // alert('MyList : '+JSON.stringify(res));
    ...
    for(var i=0;i<data.length;i++){
       ...
       dHtml+= '<a href="#" class="ui-btn edit-btns" id="edit_'+data[i].id+'" onclick="editList(this)"><div class="images-hidden"><img src="themes/images/editicon.png"></div>'+
        '<div class="discrip-hidden">Edit</div></a></div>'+
        '<a class="trackdetail"><div class="left-sec" onclick="getTruckDetails_AU('+data[i].id+')"><div class="image"><img  src="'+imgurl+'"></div>'+
...
function displayDashboard2(res){
    ...
    for(var i=0;i<data.length;i++){
        ...
        dHtml+= '<a href="#" class="ui-btn edit-btns" id="editD_'+data[i].id+'" onclick="editList(this)"><div class="images-hidden"><img src="themes/images/editicon.png"></div>'+
        '<div class="discrip-hidden">Edit</div></a></div>'+
        '<a class="trackdetail"><div class="left-sec" onclick="getTruckDetails_AU('+data[i].id+')"><div class="image"><img  src="'+imgurl+'"></div>'+
        '<div class="discrip"><h3 class="dis-truck">'+(data[i].headline.length < 22 ? data[i].headline : data[i].headline.substr(0, 22) + '<br>'+data[i].headline.substr(22, data[i].headline.length))+'</h3><div class="small-discrip">'+data[i].cat_1_name+'/'+data[i].cat_2_name+'</div><div class="truckrate">$ '+data[i].price+'</div></div></div>';
        if(data[i].publishStatus=='Published'){
    ...
    $.ajax({
           ...
           success: function(res){
                  //  alert(JSON.stringify(res));
                    console.log('del success'+JSON.stringify(res))
